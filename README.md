...
```text
7z.exe a -tzip -mm=Deflate -mmt=on -mx9 -mfb=128 -mpass=10 -mcu=on -bb0 <archive-path> @<list-file>
```

`@<list-file>` is a temporary file generated by the script that contains one source path per line. This avoids Windows command-line length limits when many files are archived at once.
